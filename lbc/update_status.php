...
if (!$ticket_id || !in_array($action, $allowed_actions)) {
...
try {
    ...
    if ($old_status !== $new_status) {
        ...
        if ($new_status === 'confirmed' && $old_status !== 'confirmed') {
            $stmt = $conn->prepare("UPDATE events SET available_tickets = available_tickets - 1 WHERE id = ? AND available_tickets > 0");
